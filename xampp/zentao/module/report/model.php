...
EOT;
     ...
     */
    public function getDataOfBugAssignedTo()
    {
        $datas = $this->dao->select('t2.account, t2.realname AS name, count(t2.realname) AS value')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')
            ->on('t1.assignedTo = t2.account')
     ...
     * 2013-09-24 add by fujia
     */
    public function getDataOfBugOpenedBy()
    {
        $datas = $this->dao->select('t2.account, t2.realname AS name, count(t2.realname) AS value')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')
     ...
     */
    public function getAssignedToPairs()
    {
        $datas = $this->dao->select('t2.account, t2.realname AS name')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')
            ->on('t1.assignedTo = t2.account')
            ->where('t1.status')->eq('active')
            ...
            ->andWhere('t2.deleted')->eq(0)
            ->groupBy('name')
            ->orderBy('count(t2.realname) DESC')
            ->limit(10)
            ->fetchPairs();
        if(!$datas) return array();
        return $datas;
    ...
    /**
     * coship start
     * 2013-10-09 add by fujia
     */
    public function getOpenedByPairs()
    {
        $datas = $this->dao->select('t2.account, t2.realname AS name')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_USER)->alias('t2')
     ...
     * 2013-10-08 add by fujia
     */
    public function getBugsByAssignedTo($assignedTo)
    {
        $datas = $this->dao->select('t1.assignedTo, t2.name AS product, t1.id, t1.title, t1.severity, t1.type, t1.status, t1.confirmed, t3.realname AS openedBy')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_PRODUCT)->alias('t2')
            ->on('t1.product = t2.id')
            ->leftJoin(TABLE_USER)->alias('t3')
            ->on('t1.openedBy = t3.account')
            ->where('t1.assignedTo')->in($assignedTo)
            ->andWhere('t1.status')->eq('active')
            ->andWhere('t1.deleted')->eq(0)
            ->orderBy('t1.assignedTo')
            ->fetchAll();
        if(!$datas) return array();
        return $datas;
    }
     ...
     * 2013-10-09 add by fujia
     */
    public function getBugsByOpenedBy($openedBy)
    {
        $datas = $this->dao->select('t1.openedBy, t2.name AS product, t1.id, t1.title, t1.severity, t1.type, t1.status, t1.confirmed, t3.realname AS assignedTo')
            ->from(TABLE_BUG)->alias('t1')
            ->leftJoin(TABLE_PRODUCT)->alias('t2')
            ->on('t1.product = t2.id')
            ->leftJoin(TABLE_USER)->alias('t3')
            ->on('t1.assignedTo = t3.account')
            ->where('t1.openedBy')->in($openedBy)
            ->andWhere('t1.status')->eq('active')
            ->andWhere('t1.deleted')->eq(0)
            ->orderBy('t1.openedBy')
            ->fetchAll();
     ...
     */
    public function getDataOfProductsProjects($beginDate, $endDate)
    {
        $datas = array();
        $i = 0;
        ...
        $products = $this->dao->select('id, name')->from(TABLE_PRODUCT)
            ->where('status')->eq('normal')  // 产品状态正常 2014-01-06 fujia
            ->andWhere('deleted')->eq(0)
            ->fetchAll();
        foreach($products as $product)
        {
            $scope    = $this->getDataOfProductScope($product->id, $beginDate, $endDate);
            $schedule = $this->getDataOfProductSchedule($product->id, $beginDate, $endDate);
            $projects = $this->getProjectsByProductId($product->id, $beginDate, $endDate);
            ...
            if($projects)
            {
                ...
                {
                    $quality = $this->getDataOfProjectQuality($project->id, $beginDate, $endDate);
                    $datas[$i]['projects'][$j]['id']           = $project->id;
                    $datas[$i]['projects'][$j]['name']         = $project->name;
                    $datas[$i]['projects'][$j]['testtasks']    = $quality['testtasks'];
                    $datas[$i]['projects'][$j]['bugs']         = $quality['total'];
                    $datas[$i]['projects'][$j]['resolvedBugs'] = $quality['resolvedNum'];
                    $totalTesttasks = $totalTesttasks + $quality['testtasks'];
                    $j++;
                }

                $datas[$i]['productId']       = $product->id;
                $datas[$i]['productName']     = $product->name;
                $datas[$i]['stories']         = $scope['total'];
                $datas[$i]['releasedStories'] = $scope['releasedNum'];
                $datas[$i]['goingOnStories']  = $scope['goingOnNum'];
                $datas[$i]['plans']           = $schedule['total'];
                $datas[$i]['releasedPlans']   = $schedule['releasedNum'];
                $datas[$i]['totalTesttasks']  = $totalTesttasks;
                $i++;
            }
        }
     ...
     * coship start
     * 产品关联的项目
     * 2013-09-26 add by fujia
     */
    public function getProjectsByProductId($productId, $beginDate, $endDate)
    {
        $datas = $this->dao->select('t1.id, t1.name')
            ->from(TABLE_PROJECT)->alias('t1')
            ->leftJoin(TABLE_PROJECTPRODUCT)->alias('t2')
            ->on('t1.id = t2.project')
            ->where('t1.status')->eq('doing')  // 项目状态进行中 2014-01-06 fujia
            ->andWhere('t1.deleted')->eq(0)
     ...
     * 2013-09-26 add by fujia
     */
    public function getDataOfProductScope($productId, $beginDate, $endDate)
    {
        $datas = array();
        /* 需求总数 */
        $total = $this->dao->select('count(product) as sum')
            ->from(TABLE_STORY)
            ->where('product')->eq($productId)
            ->andWhere('openedDate')->gt($beginDate)
            ->andWhere('openedDate')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->fetch();
    
        /* 需求发布数 */
        $releasedNum = $this->dao->select('count(product) as sum')
            ->from(TABLE_STORY)
            ->where('product')->eq($productId)
            ->andWhere('openedDate')->gt($beginDate)
            ->andWhere('openedDate')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->andWhere('stage')->in('developed, testing, tested, verified, released')
            ->fetch();

        /* 进行中需求数 */
        $goingOnNum = $this->dao->select('count(product) as sum')
            ->from(TABLE_STORY)
            ->where('product')->eq($productId)
            ->andWhere('openedDate')->gt($beginDate)
            ->andWhere('openedDate')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->andWhere('stage')->in('planned, projected, developing')
            ->fetch();

        $datas['total']      = $total->sum;
        $datas['releasedNum'] = $releasedNum->sum;
        $datas['goingOnNum']  = $goingOnNum->sum;
     ...
     * 2013-09-26 add by fujia
     */
    public function getDataOfProductSchedule($productId, $beginDate, $endDate)
    {
        $datas = array();
        ...
        $total = $this->dao->select('count(product) as sum')
            ...
            ->where('product')->eq($productId)
            ->andWhere('end')->gt($beginDate)
            ->andWhere('end')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->fetch();
        ...
        $releasedNum = $this->dao->select('count(product) as sum')
            ->from(TABLE_RELEASE)
            ->where('product')->eq($productId)
            ->andWhere('date')->gt($beginDate)
            ->andWhere('date')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->fetch();
     ...
     * 2013-09-26 add by fujia
     */
    public function getDataOfProjectQuality($projectId, $beginDate, $endDate)
    {
        $datas = array();

        /* 提交测试数 */
        $testtasks = $this->dao->select('count(project) as sum')
            ->from(TABLE_TESTTASK)
            ->where('project')->eq($projectId)
            ->andWhere('begin')->gt($beginDate)
            ->andWhere('begin')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->fetch();

        /* Bug总数 */
        $total = $this->dao->select('count(project) as sum')
            ->from(TABLE_BUG)
            ->where('project')->eq($projectId)
            ->andWhere('openedDate')->gt($beginDate)
            ->andWhere('openedDate')->lt($endDate)
        ...
        /* 解决Bug数 */
        $resolvedNum = $this->dao->select('count(project) as sum')
            ->from(TABLE_BUG)
            ->where('project')->eq($projectId)
            ->andWhere('resolvedDate')->gt($beginDate)
            ->andWhere('resolvedDate')->lt($endDate)
            ->andWhere('deleted')->eq(0)
            ->andWhere('status')->in('resolved, closed')
            ->fetch();

        $datas['testtasks']   = $testtasks->sum;
        $datas['total']       = $total->sum;
        $datas['resolvedNum'] = $resolvedNum->sum;
     ...
     */
    public function getBugMonths($field, $null = 'false')
    {
        $datas = $this->dao->select("distinct(left($field, 7)) as month")
            ->from(TABLE_BUG)
            ->beginIF($null == 'false')->where($field)->ne('0000-00-00')->fi()
            ->orderBy('month')
            ->fetchAll();
     ...
     * 2013-09-27 add by fujia
     */
    public function getOpenedBugs($month)
    {
        $datas = $this->dao->select('count(*) as sum')
            ->from(TABLE_BUG)
            ->where('openedDate')->like($month.'%')
            ->andWhere('deleted')->eq(0)
            ->fetch();

        $total = empty($datas) ? 0 : $datas->sum;
        return $total;
    }
     ...
     * 2013-09-27 add by fujia
     */
    public function getResolvedBugs($month)
    {
        $datas = $this->dao->select('count(*) as sum')
            ->from(TABLE_BUG)
            ->where('resolvedDate')->like($month.'%')
            ->andWhere('status')->in('resolved, closed')
            ->andWhere('deleted')->eq(0)
            ->fetch();
