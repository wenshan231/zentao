...
<table class='cont-rt5'> 
  ...
  <tr valign='top'>
    ...
    <td class="side">
      ...
      <!-- coship start 2013-08-29 add by fujia 增加相关测试任务模块 -->
      <fieldset>
        <?php foreach($testtasks as $testtask):?>
        <?php $testtaskLink = $this->createLink('testtask', 'view', "taskID=$testtask->id", '', true);?>
        <legend><?php echo $lang->build->linkTesttask;?></legend>
        <table class='table-1 a-left'>
          <tr>
            <th width='25%' class='a-right'><?php echo $lang->testtask->name;?></th>
            <td><?php echo html::a($testtaskLink, $testtask->name, '', "class='preview'");?></td>
          </tr>  
          <tr>
            <th class='rowhead'><?php echo $lang->testtask->status;?></th>
            <td><?php echo $lang->testtask->statusList[$testtask->status];?></td>
          </tr>  
          <tr>
            <th class='rowhead'><?php echo $lang->testtask->desc;?></th>
            <td><?php echo $testtask->desc;?></td>
          </tr>  
          <tr>
            <th class='rowhead'><?php echo $lang->testtask->report;?></th>
            <td><?php echo $testtask->report;?></td>
          </tr>  
        </table>
        <?php endforeach;?>
      </fieldset>
