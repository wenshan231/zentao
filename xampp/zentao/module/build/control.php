...
class build extends control
{
    ...
    public function create($projectID)
    {
        ...
        */
        $orderBy = 'status_asc, stage_asc';
        $stories = $this->story->getProjectLinkedStories($projectID, $orderBy);
        $bugs    = $this->bug->getProjectLinkedBugs($projectID); 
		/* coship end */
        /* Assign. */
    ...
    public function view($buildID)
    {
        ...
        /**
         * coship start
         * 2013-08-30 add by fujia
         */
        $testtasks = $this->dao->select('t1.id, t1.name, t1.status, t1.desc, t1.report')
            ->from(TABLE_TESTTASK)->alias('t1')
            ->leftJoin(TABLE_BUILD)->alias('t2')->on('t1.build = t2.id')
            ->where('t2.id')->eq($buildID)
            ->fetchAll();
        $this->loadModel('common')->saveQueryCondition($this->dao->get(), 'testtask');
        /* coship end */
