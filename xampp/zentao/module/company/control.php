 ...
 * @package     company
 * @version     $Id: control.php 4714 2013-05-02 08:10:58Z chencongzhi520@gmail.com $
...
class company extends control
{
    ...
    public function browse($param = 0, $type = 'bydept', $orderBy = 'id', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        /**
         * coship start
         * 2013-12-10 update by fujia
         */
        if($type == 'bydept' || $type == 1)
        /* coship end */
        ////if($type == 'bydept')
        {
        ...
        if($type ==1 )
        {
            $userNames = $this->loadModel('user')->getPairs('nodeleted|noclosed');
            $userName = $userNames[$param];
            $pos=strpos($userName,":");
            $userName=substr($userName,$pos+1);

            $users = $this->dept->getUsers($childDeptIds);
         ...
         * 2013-08-05 add by fujia
         */
        $tmpUserTasks = $this->dept->getUserTasks($childDeptIds);
        $tasksMap = array();
        foreach($tmpUserTasks as $t) {
