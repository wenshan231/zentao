...
class deptModel extends model
{
     ...
     * @return array
     */
    public function getUsers($deptID, $pager = null, $orderBy = 'd.id, u.id')
    {
...
////            ->beginIF($deptID)->andWhere('dept')->in($deptID)->fi()
        /**
         * coship start
         * 2013-08-06 update by fujia
         */
        if($orderBy != 'd.id, u.id') $orderBy = 'u.' . $orderBy;
        return $this->dao->select('u.*, d.name as deptName')->from(TABLE_USER)->alias('u')
            ->leftJoin(TABLE_DEPT)->alias('d')->on('u.dept = d.id')
            ->where('u.deleted')->eq(0)
            ->beginIF($deptID)->andWhere('u.dept')->in($deptID)->fi()
        /* coship end */
            ->orderBy($orderBy)
            ->page($pager)
            ->fetchAll();
     ...
     */
    public function getUserTasks($deptID) 
	{
		//u.account, t.id as taskId, t.name as taskName
        return $this->dao->select('u.*, t.*, zt_project.name as projectName')->from(TABLE_USER)->alias('u')
			->leftJoin(TABLE_TASK)->alias('t')->on('u.account = t.assignedTo')
			->leftjoin(TABLE_PROJECT)->on('zt_project.id = t.project')
            ->where('u.deleted')->eq(0)
            ->andWhere('t.deleted')->eq(0)
            ->beginIF($deptID)->andWhere('u.dept')->in($deptID)->fi()
			->andWhere('t.status')->eq('doing')
            ->orderBy('u.account')
            ->fetchAll();
	}
    /* coship end */
