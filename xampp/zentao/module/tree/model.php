 ...
 * @package     tree
 * @version     $Id: model.php 4608 2013-03-15 02:46:52Z wyd621@gmail.com $
...
class treeModel extends model
{
    ...
    /**
     * coship start
     * 2013-10-11 add by fujia
     * Create link of a notice.
     * 
     * @param  object   $module 
     * @access public
     * @return string
     */
    public function createNoticeLink($module)
    {
        $linkHtml = html::a(helper::createLink('notice', 'browse', "moduleID={$module->id}"), $module->name, '_self', "id='module{$module->id}'");
        return $linkHtml;
    }
    /* coship end */
     ...
     * @return string
     */
    public function getCaseTreeMenu($productID, $taskID)
    {
        $productCases = $this->loadModel('testtask')->getLinkCasesTotal($taskID);
        $treeMenu = array();
        $stmt = $this->dbh->query($this->buildMenuQuery($productID, 'case', 0));
        while($module = $stmt->fetch())
        {
            $linkHtml = call_user_func(array('treeModel', 'createTestTaskLink'), $module, $taskID);
            ...
            if(isset($treeMenu[$module->id]) and !empty($treeMenu[$module->id]))
            {
                ...
                /* 父模块case数统计。 */
                $parent[$module->id]['wait']  = isset($total[$module->id]) ? $total[$module->id]['wait'] : 0;
                $parent[$module->id]['block'] = isset($total[$module->id]) ? $total[$module->id]['block'] : 0;
                $parent[$module->id]['fail']  = isset($total[$module->id]) ? $total[$module->id]['fail'] : 0;
                $parent[$module->id]['pass']  = isset($total[$module->id]) ? $total[$module->id]['pass'] : 0;
                if(isset($productCases[$module->id]))
                {
                    $parent[$module->id]['wait']  += $productCases[$module->id]['waitTotal'];
                    $parent[$module->id]['block'] += $productCases[$module->id]['blockedTotal'];
                    $parent[$module->id]['fail']  += $productCases[$module->id]['failTotal'];
                    $parent[$module->id]['pass']  += $productCases[$module->id]['passTotal'];
                }
                if($parent[$module->id])
                {
                    $treeMenu[$module->parent] .= "<li>$linkHtml"; 
                    $treeMenu[$module->parent] .= '( <span>'.$parent[$module->id]['wait'].'</span>, <span style="color:blue;">'.$parent[$module->id]['block'].'</span>, <span style="color:red;">'.$parent[$module->id]['fail'].'</span>, <span style="color:green;">'.$parent[$module->id]['pass'].'</span> )';

                    /* 父级模块累加。 */
                    if(isset($total[$module->parent]))
                    {
                        $total[$module->parent]['wait']  += $parent[$module->id]['wait'];
                        $total[$module->parent]['block'] += $parent[$module->id]['block'];
                        $total[$module->parent]['fail']  += $parent[$module->id]['fail'];
                        $total[$module->parent]['pass']  += $parent[$module->id]['pass'];
                    }
                    else
                    {
                        $total[$module->parent]['wait']  = $parent[$module->id]['wait'];
            ...
            else
            {
                if(isset($productCases[$module->id]))
                {
                    ...
                    $treeMenu[$module->parent] .= '( <span>'.$productCases[$module->id]['waitTotal'].'</span>, <span style="color:blue;">'.$productCases[$module->id]['blockedTotal'].'</span>, <span style="color:red;">'.$productCases[$module->id]['failTotal'].'</span>, <span style="color:green;">'.$productCases[$module->id]['passTotal'].'</span> )';

                    /* 父级模块累加。 */
                    if(isset($total[$module->parent]))
                    {
                        $total[$module->parent]['wait']  += $productCases[$module->id]['waitTotal'];
                        $total[$module->parent]['block'] += $productCases[$module->id]['blockedTotal'];
                        $total[$module->parent]['fail']  += $productCases[$module->id]['failTotal'];
                        $total[$module->parent]['pass']  += $productCases[$module->id]['passTotal'];
                    }
                    else
                    {
                        $total[$module->parent]['wait']  = $productCases[$module->id]['waitTotal'];
            ...
            if(isset($treeMenu[$module->parent])) $treeMenu[$module->parent] .= "</li>\n"; 
        }
