 ...
 * @package     task
 * @version     $Id: control.php 4683 2013-04-27 05:08:48Z chencongzhi520@gmail.com $
...
class task extends control
{
     ...
     */
    public function create($projectID = 0, $storyID = 0, $moduleID = 0, $taskID = 0, $bugID = 0)
    /* coship end */
    ////public function create($projectID = 0, $storyID = 0, $moduleID = 0, $taskID = 0)
    {
         ...
         * 2013-08-28 update by fujia
         * 判断是否有bugID的传入，如果有，连接数据库进行取出BUG相关的任何字段信息，然后使用$bug->title获取bug标题信息
         */
        ...
        if($bugID != 0)
        {
            $task->name = $bug->title;
            $task->type = 'devel'; 
            $task->assignedTo = $bug->assignedTo; //bugower
            $bugIDtask  = html::a(helper::createLink('bug', 'view', "bugID=$bugID"), $bugID);
            $task->desc = $bugIDtask . '<br/>' . $bug->steps;
        }
        /* coship end */
        ...
        if(!empty($_POST))
        {
             ...
             * coship start
             * 2013-08-05 update by fujia
             * 如果有bugID，则在数据库操作的时候，需要特殊写入其他的字段
             */
            $tasksID = $this->task->create($projectID, $bugID);
            /* coship end */
    ...
    public function customFields()
    {
        if($_POST)
        {
            ...
            setcookie('taskFields', $customFields, $this->config->cookieLife, $this->config->webRoot);
            die(js::reload('parent'));
        }

        $customFields = $this->cookie->taskFields ? $this->cookie->taskFields : $this->config->task->list->defaultFields;

        $this->view->allFields     = $this->task->getFieldPairs($this->config->task->list->allFields);
        $this->view->customFields  = $this->task->getFieldPairs($customFields);
        $this->view->defaultFields = $this->task->getFieldPairs($this->config->task->list->defaultFields);
        die($this->display());
    }
    /* coship end */
}
