...
$(document).ready(function()
{
     ...
     */
    $("#showTasks").colorbox({width:960, height:500, iframe:true, transition:'elastic', speed:350, scrolling:true});
    /* coship end */
});
...
function showTaskButton()
{
    if($('#assignedTo').val())
    {
        taskLink = createLink('company', 'browse', "deptID=" + $('#assignedTo').val() + "& userFlag=1");
        $('#showTasks').removeClass('hidden');
        $('#showTasks').attr('href', taskLink);
    }
    else
    {
        $('#showTasks').addClass('hidden');
    }
}
/* coship end */