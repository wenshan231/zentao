 ...
 * @package     task
 * @version     $Id: model.php 4740 2013-05-04 01:01:42Z chencongzhi520@gmail.com $
...
class taskModel extends model
{
     ...
     */
    public function create($projectID, $bugID=0)
    /* coship end */
    ////public function create($projectID)
    {
        ...
        foreach($this->post->assignedTo as $assignedTo)
        {
            $task = fixer::input('post')
                ...
                /**
                 * coship start
                 * 2014-02-22 update by shukaiming
                 * 创建任务时把估计时间和剩余时间默认修改为7.5h
                 */
                ->setDefault(' story', 0)
                ->setDefault('estimate, left', 7.5)
               /* coship end */
             ...
             * 插入bug和task的关联关系
             */
            if($bugID != '0')$this->dao->update(TABLE_BUG)->set('toTask')->eq($taskID)->where('id')->eq($bugID)->exec();
            if($bugID != '0')$this->dao->update(TABLE_TASK)->set('fromBug')->eq($bugID)->where('id')->eq($taskID)->exec();
            $actionID = $this->loadModel('action')->create('task', $taskID, 'Opened', '');
            $this->action->create('bug', $bugID, 'Totask', '', $taskID);
            /* coship end */
        }
        return $tasksID;
    }
    ...
    public function recordEstimate($taskID)
    {
        ...
        /** coship start
         * 2014-1-8 add/update by shukaiming
         */
		$now      = helper::now();
        /* coship end */
        $estimates = array();
    ...
    public function getFieldPairs($fields)
    {
        ...
        {
            $field = trim($field);
            $fields[$field] = $this->lang->task->$field;
            unset($fields[$key]);
        }
        return $fields;
    }
     ...
     * @return array
     */
    public function formCustomedTasks($tasks)
    {
        foreach($tasks as $task)
        {
            /* Get related objects title or names. */
            $relatedModule  = $this->dao->select('id, name')->from(TABLE_MODULE)->where('id')->eq($task->module)->fetchPairs();
            $relatedProject = $this->dao->select('id, name')->from(TABLE_PROJECT)->where('id')->eq($task->project)->fetchPairs();
            $relatedStory   = $this->dao->select('id, title')->from(TABLE_STORY) ->where('id')->eq($task->story)->fetchPairs();
            $relatedBug     = $this->dao->select('id, title')->from(TABLE_BUG)->where('id')->eq($task->fromBug)->fetchPairs();

            /* fill some field with useful value. */
            if(isset($relatedModule[$task->module]))   $task->module  = $relatedModule[$task->module];
            if(isset($relatedProject[$task->project])) $task->project = $relatedProject[$task->project];
            if(isset($relatedStory[$task->story]))     $task->story   = $relatedStory[$task->story];
            if(isset($relatedBug[$task->fromBug]))     $task->fromBug = $relatedBug[$task->fromBug];
        }
        return $tasks;
    }
    /* coship end */
