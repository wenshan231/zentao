 ...
 * @package     story
 * @version     $Id: model.php 4738 2013-05-04 00:37:33Z chencongzhi520@gmail.com $
...
class storyModel extends model
{
     ...
     */
    public function getProjectLinkedStories($projectID = 0, $orderBy = 'pri_asc,id_desc')
    {
    	$stories = $this->dao->select('t1.*, t2.*')->from(TABLE_PROJECTSTORY)->alias('t1')
    	->leftJoin(TABLE_STORY)->alias('t2')->on('t1.story = t2.id')
    	->where('t1.project')->eq((int)$projectID)
    	->andWhere('t2.deleted')->eq(0)
    	->andWhere('t2.stage')->ne('released')
    	->orderBy($orderBy)
    	->fetchAll('id');
    	return $stories;
    }
