...
<!-- coship start 2013-10-21 add by fujia -->
<style>
.history p{margin:0}
</style>
<!-- coship end -->
    ...
    <li value='<?php echo $i ++;?>'>
      ...
      <?php if(!empty($action->comment)) echo "<div class='history'>";?>
        ...
        <?php echo $this->action->printChanges($action->objectType, $action->history);?>
         ...
         /**
          * coship start
          * 2013-10-21 update by fujia
          */
         if($action->comment)
         {
            $strComment = html::convertSimpleUbb2Html($action->comment);
            echo preg_replace("/<\/p>[\n\r\t\v]{1,}<br>/", "</p>", $strComment);
         }
         /* coship end */
         ////if($action->comment) echo nl2br($action->comment);
         ?>
...
<script><?php echo '$("#historyItem li").reverseOrder();'; ?> </script>
<!-- coship end -->
