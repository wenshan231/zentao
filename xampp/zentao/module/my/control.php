 ...
 * @package     dashboard
 * @version     $Id: control.php 4736 2013-05-03 08:30:57Z chencongzhi520@gmail.com $
...
class my extends control
{
    ...
    public function index()
    {
        ...
        $this->view->projectStats  = $projectStats;//1.25s->1.59s
        $this->view->productStats  = $productStats;//1.59s->1.67s
        $this->view->actions       = $this->loadModel('action')->getDynamic('all', 'latest3days', 'date_desc', $pager);//1.67s->2.46s
        $this->view->todos         = $this->loadModel('todo')->getList('all', $account, 'wait, doing', $this->config->my->todoCounts);
        $this->view->tasks         = $this->loadModel('task')->getUserTasks($account, 'assignedTo', $this->config->my->taskCounts);//2.44s->2.53s
    ...
    public function bug($type = 'assignedTo', $orderBy = 'id_desc', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
        ...
        {
            if(isset($projectsStatus[$bug->project])) $bug->projectStatus = $projectsStatus[$bug->project];
        }
        /* coship end */
