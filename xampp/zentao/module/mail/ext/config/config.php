<?php
$config->mail->turnon      = 1;    // trun on email feature or not.
$config->mail->fromAddress = 'zentao@coship.com';       // The from address.
...
if($config->mail->mta == 'smtp')
{
    ...
    $config->mail->smtp->auth     = 1;       // Need auth or not.
    $config->mail->smtp->host     = 'webmail.coship.com';         // The smtp server host address.
    $config->mail->smtp->port     = '25';         // The smtp server host port.
    $config->mail->smtp->secure   = '';         // The type to encode datas, 'ssl' or 'tls' allowed
    $config->mail->smtp->username = 'zentao@coship.com';         // The smtp user, may be a full email adress.
    $config->mail->smtp->password = '906602';         // The smtp user's password.
}
