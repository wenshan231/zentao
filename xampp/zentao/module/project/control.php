 ...
 * @package     project
 * @version     $Id: control.php 4712 2013-05-02 07:55:43Z wyd621@gmail.com $
...
class project extends control
{
    ...
    {   
         ...
         * Get custom fields. 
         */
        $customFields = $this->cookie->taskFields != false ? $this->cookie->taskFields : $this->config->task->list->defaultFields;
        $customed     = !($customFields == $this->config->task->list->defaultFields);
 
        /* If customed, get related name or titles. */
        if($customed) $tasks = $this->task->formCustomedTasks($tasks);
    ...
    public function team($projectID = 0)
    {
         ...
         */
        $this->loadModel('task');
        $tmpTeamTasks = $this->project->getTeamTasks($projectID);
        $tmpTeamTasks = $this->task->processTasks($tmpTeamTasks);
        $tasksMap = array();		
        foreach($tmpTeamTasks as $t) {
            $tasksMap[$t->assignedTo][] = $t;
        }
        $this->view->tasksMap = $tasksMap;	
        /* coship end */
