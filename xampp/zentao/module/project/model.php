 ...
 * @package     project
 * @version     $Id: model.php 4741 2013-05-04 01:54:07Z wyd621@gmail.com $
...
class projectModel extends model
{
    ...
    public function select($projects, $projectID, $currentModule, $currentMethod, $extra = '')
    {
         ...
         */
        $productNames = array_keys($selectGroup);
 		$this->sortByGBK($productNames);
 		$productNames = array_flip($productNames);
 		unset($productNames[$this->lang->project->noProduct]);
 		foreach ($productNames as $k=>$v) {	
 				$productNames[$k] = $selectGroup[$k];
 		}
 		$productNames[$this->lang->project->noProduct]=$selectGroup[$this->lang->project->noProduct];
 		$selectGroup = $productNames;
 		/* coship end */
    ...
    public function getProductGroupList()
    {
        $list = $this->dao->select('t1.id, t1.name,t1.status, t2.product')->from(TABLE_PROJECT)->alias('t1')
			 ...
			 */
            ->orderBy('convert(t1.name using gbk)')
            ->fetchGroup('product');
			/* coship end */
        $noProducts = array();
     ...
     * 2013-08-05 update by fujia
     */
    public function getProjectStats($status = 'undone', $productID = 0, $itemCounts =30 )//// 2014-02-24 add by shukaiming 把$itemCounts从10000修改为30
    /* coship end */
    ...
    /**
     * coship start
     * 2013-08-07 add by fujia
     */
    public function getTeamTasks($projectID) 
    {
        return $this->dao->select('zt_task.*, zt_project.name as projectName')->from(TABLE_TEAM)
            ->leftJoin(TABLE_TASK)->on('zt_team.account = zt_task.assignedTo')
            ->leftjoin(TABLE_PROJECT)->on('zt_project.id = zt_task.project')
            ->where('zt_team.project')->eq((int)$projectID)
            ->andWhere('zt_task.status')->eq('doing')
            ->orderBy('assignedTo')
            ->fetchAll();
    }
     ...
     * 2014-01-21 add/update by shukaiming
     */
	public function sortByGBK(&$array) 
	{
