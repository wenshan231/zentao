...
class testtaskModel extends model
{
     ...
     * @return array
     */
    public function getLinkCasesTotal($taskID, $type = 'module')
    {
        $groupBy = ($type == 'assignedTo') ? "t1.assignedTo" : "t2.$type";

        /* 关联测试用例总数。 */
        $cases = $this->dao->select("$groupBy, count(*) AS total")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
            ->groupBy($groupBy)
            ->fetchPairs();
        ...
        /* 未执行的测试用例数。 */
        $waitTotal = $this->dao->select("$groupBy, count(*) AS waitTotal")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
        ...
        /* 已完成的测试用例数。 */
        $doneTotal = $this->dao->select("$groupBy, count(*) AS doneTotal")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
        ...
        /* 执行通过的。 */
        $passTotal = $this->dao->select("$groupBy, count(*) AS passTotal")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
            ->andwhere('t1.lastRunResult')->eq('pass')
            ->groupBy($groupBy)
            ->fetchPairs();

        /* 执行失败的。 */
        $failTotal = $this->dao->select("$groupBy, count(*) AS failTotal")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
            ->andwhere('t1.lastRunResult')->eq('fail')
            ->groupBy($groupBy)
            ->fetchPairs();

        /* 执行阻塞的。 */
        $blockedTotal = $this->dao->select("$groupBy, count(*) AS blockedTotal")->from(TABLE_TESTRUN)->alias('t1')
            ->leftJoin(TABLE_CASE)->alias('t2')->on('t1.case = t2.id')
            ->where('t1.task')->eq((int)$taskID)
            ->andwhere('t1.lastRunResult')->eq('blocked')
            ->groupBy($groupBy)
            ->fetchPairs();
        ...
        if($cases)
        {
            ...
            {
                $casesTotal[$key]['total']        = $type;
                $casesTotal[$key]['waitTotal']    = isset($waitTotal[$key])    ? $waitTotal[$key]    : 0;
                $casesTotal[$key]['doneTotal']    = isset($doneTotal[$key])    ? $doneTotal[$key]    : 0;
                $casesTotal[$key]['passTotal']    = isset($passTotal[$key])    ? $passTotal[$key]    : 0;
                $casesTotal[$key]['failTotal']    = isset($failTotal[$key])    ? $failTotal[$key]    : 0;
                $casesTotal[$key]['blockedTotal'] = isset($blockedTotal[$key]) ? $blockedTotal[$key] : 0;
            }
        }
     ...
     * @return array
     */
    public function getUserPairs($accounts)
    {
