...
<div id='titlebar'>
  ...
  <div>
     ...
     */
    $bugtotaskParams = "projectID=$bug->project&storyID=$bug->story&moduleID=0&taskID=0&bugID=$bug->id";
    /* coship end */
    if(!$bug->deleted)
    {
        ob_start();
		 ...
		 */
		if($bug->projectStatus != 'suspended' || $bug->status == 'closed')
		{
        /* coship end */
         ...
         * 增加bug转任务按钮
         */
        common::printLink('task', 'create', "$bugtotaskParams", $lang->bug->importtotask); 
        /* coship end */
        common::printDivider();
  ...
  <tr valign='top'>
    <td>
      <fieldset>
		...
		<!-- coship start 2014-02-07 update by shukaiming -->
      ...
      <fieldset>
        ...
        <table class='table-1 a-left'>
          ...
          <tr>
            ...
            <!-- coship start 2013-10-18 add by fujia -->
            <?php if($bug->projectStatus == 'suspended' && $bug->status != 'closed'):?>
            <td><span style="background:#FFFF77;display:block;float:left;width:50px;"><strong><?php echo $lang->project->suspend;?></strong></span></td>
            <?php else:?>
            <!-- coship end -->
          ...
          <tr>
            ...
            <!-- coship start add by shukaiming 2014-02-25 在view页面增加重现概率显示 -->
            <tr>
                <td class='rowhead'><?php echo $lang->bug->recurrenceRate;?></td>
                <td><?php echo $bug->recurrenceRate;?></td>
            </tr>
            <!-- coship end -->
          <tr>
