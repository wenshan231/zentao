 ...
 * @package     bug
 * @version     $Id: control.php 4651 2013-04-15 06:17:19Z chencongzhi520@gmail.com $
...
class bug extends control
{
    ...
    public function browse($productID = 0, $browseType = 'byModule', $param = 0, $orderBy = '', $recTotal = 0, $recPerPage = 20, $pageID = 1)
    {
         ...
         * coship start
         * 项目挂起时，bug也挂起
         * 2013-10-18 add by fujia
         */
        $projectIdList = array();
        foreach($bugs as $bug) $projectIdList[$bug->id] = $bug->project;
        $projectsStatus = $this->dao->select('id, status')->from(TABLE_PROJECT)->where('id')->in($projectIdList)->fetchPairs();
        foreach($bugs as $key => $bug) 
        {
            $bug->projectStatus = isset($projectsStatus[$bug->project]) ? $projectsStatus[$bug->project] : '';
        }
        /* coship end */
    ...
    public function create($productID, $extras = '')
    {
        ...
        /**
         * coship start
         * 2014-02-20 add by shukaiming
         */
        $userId                     = $this->app->user->id;
       $this->view->browserDiys  = $this->dao->select('distinct browserdiy')->from(TABLE_BUG)->fetchPairs('browserdiy','browserdiy');
        /* coship end */
        ...
        /**
         * coship start 
         * 2014-02-07 update add/update by shukaiming
         */
        $steps      = $this->lang->bug->tplPreCondition.$this->lang->bug->tplStep. $this->lang->bug->tplResult. $this->lang->bug->tplExpect;
        /* coship end */
        $os         = '';
        $browser    = '';
        /**
         * coship start
         * 2014-02-14 add by shukaiming
         */
        $recurrenceRate = '';
        /* coship end */
         ...
         */
        $this->view->recurrenceRate   = $recurrenceRate;
        //$this->view->browserDiys  = array('360se'=>'360se','qq'=>'qq');//$this->bug->getBrowserDiyPairs($this->app->user->id);
        /* coship end */
    ...
    public function view($bugID)
    {
        ...
        $projectStatus = $this->dao->select('id, status')->from(TABLE_PROJECT)->where('id')->eq($bug->project)->fetchPairs();
        $bug->projectStatus = $projectStatus[$bug->project];
        /* coship end */
    ...
    public function resolve($bugID)
    {
        ...
        /**
         * coship start
         * 2014-02-26 add by shukaiming 获得打开当前Bug的account
         */
        $openedBy        =$bug->openedBy;
        $boolTag = ($this->user->getUserGroupId($openedBy) != '200004');
        $this->view->boolTag =$boolTag;
        /* coship end */
