...
int Volume::formatVol() {
    ...
    dev_t partNode = MKDEV(MAJOR(diskNode), (formatEntireDevice ? 0 : mPartIdx));
...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        gid = AID_SDCARD_RW;//add by duyouhua
        
        bool isFatFs = true;
        bool isNtfsFS = true;
        bool isExtFs = true;		
        if(isFatFs) {
           //-----begin  modify by duyouhua
        ...
        if (isNtfsFS) {
        	//-----begin  modify by duyouhua
        	  //if (Ntfs::doMount(devicePath, "/mnt/secure/staging", false, false, AID_SYSTEM, gid, 0002, true)) {
            if (Ntfs::doMount(devicePath, "/mnt/secure/staging", false, false, AID_SYSTEM, gid, 0000, true)) {
            //-----end
        ...
        if (isExtFs) {
            if (Extfs::doMount(devicePath, "/mnt/secure/staging", false, false)) {
                SLOGE("%s failed to mount via EXTFS (%s)\n", devicePath, strerror(errno));
                isExtFs = false;
            }
        }

        if (!isFatFs && !isNtfsFS && !isExtFs) {
