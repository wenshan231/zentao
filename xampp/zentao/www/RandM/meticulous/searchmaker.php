...
<body>
<br><br><br>
<center><p><font color='#0066CC' size=6><b>互联网产品研发中心差不多事件</b></font></p></center>
<br>
...
<?php
	...
	$zentao_passwrod = "zentao_passwrod";
	...
	$strSql = "select * from zt_user where account = '$account'";  
	$sqlinfo = mysql_query($strSql,$connection_zentao) or die("查询失败！错误是".mysql_error()); 
	$userinfo = mysql_fetch_array($sqlinfo);
	$searchername = $userinfo['realname'];	
...
<div id="hotnews">
  ...
  <ul>
	<ul id="MenuBar1" class="MenuBarHorizontal">
	  ...
	  <li class="user_t"><a class="MenuBarItemSubmenu">爆料人</a>
	    <ul class="MenuBarHorizontal_select">	     
	     <?php
		$strSql = "select * from zt_user order by convert(realname using GBK)";  
		$users = mysql_query($strSql,$connection_zentao) or die("查询失败！错误是".mysql_error());
		while($user = mysql_fetch_array($users)){ 
			$username = $user['realname'];
			$account = $user['account'];
			echo "<li class='user'><a href='searchfinder.php?logon_encode=$logon_encode&account=$account'>$username</a></li>";
		}
	    ?>	     
	    </ul>
	  </li>
	  ...
	  <li class="user_t"><a class="MenuBarItemSubmenu">差不多GG/MM</a>
	    <ul class="MenuBarHorizontal_select">
	     <?php
		$strSql = "select * from zt_user order by convert(realname using GBK)"; 	     
		$users = mysql_query($strSql,$connection_zentao) or die("查询失败！错误是".mysql_error());
		while($user = mysql_fetch_array($users)){ 
			$username = $user['realname'];
			$account = $user['account'];
			echo "<li class='user'><a href='searchmaker.php?logon_encode=$logon_encode&account=$account'>$username</a></li>";
		}
	    ?>	  
	    </ul>
	  </li>
	  
	  <li class="content_t">差不多事件</li>
	...
	<?php					 
		...
		$strSql = "select * from data where maker = '$searchername'";  
		$result = mysql_query($strSql,$connection_meti) or die("查询失败！错误是".mysql_error()); 		
		...
		$strSql = "select * from data where maker = '$searchername' limit $fromdata,$page_size";
		$result = mysql_query($strSql,$connection_meti) or die("查询失败！错误是".mysql_error());		
		...
		while($row = mysql_fetch_array($result)){
			...
			echo "<li class='user_c'>$finder</li>";			
			echo "<li class='user_c'>$maker</li>";
	 ...
	 <tr>
      ...
      <?php
       ...
       echo "<a href='./add.php?logon_encode=$logon_encode'><input type='button' value='+ 新增记录' style='width:8%; height:4%; margin-left:93%;'></input></a>";
       if("$logon_decode" == "shaoying" || "$logon_decode" == "zhangchen" || "$logon_decode" == "weishujuan" || "$logon_decode" == "caofubing" || "$logon_decode" == "heping" || "$logon_decode" == "qiaoting" || "$logon_decode" == "xiaoqin" || "$logon_decode" == "tongfang")
       {
